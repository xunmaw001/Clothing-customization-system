<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="服装类型" prop="fuzhuangleixing">
          <el-input v-model="ruleForm.fuzhuangleixing" 
              placeholder="服装类型" clearable  :readonly="ro.fuzhuangleixing"></el-input>
        </el-form-item>
        <div v-else>
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `fuzhuangleixing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.fuzhuangleixingCrossAddOrUpdateFlag = false;
						 this.parent.search();
